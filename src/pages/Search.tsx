...
const STATIC_PAGES: { title: string; keywords: string[]; excerpt: string; link: string }[] = [
  {
    title: "Hakkımızda",
  ...
  const searchAllContent = async () => {
    ...
    try {
      ...
      const staticMatches = STATIC_PAGES.filter((page) =>
        ...
        page.keywords.some((kw) => kw.toLowerCase().includes(query) || query.includes(kw.toLowerCase()))
      );
